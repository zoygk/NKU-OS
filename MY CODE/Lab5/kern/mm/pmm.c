...
/* copy_range - 将一个进程 A 的内存内容（从 start 到 end）复制到另一个进程 B
 * @to:    进程 B 的页目录的地址
 * @from:  进程 A 的页目录的地址
 * @share: 标志位，用于指示是复制（dup）还是共享（share）。我们这里使用的是复制方法，因此该标志未被使用。
 *
 * 调用关系：copy_mm --> dup_mmap --> copy_range
 */
int copy_range(pde_t *to, pde_t *from, uintptr_t start, uintptr_t end, bool share) {
    // 确保 start 和 end 都是页面对齐的地址
    assert(start % PGSIZE == 0 && end % PGSIZE == 0);
    // 确保给定的地址范围是用户可访问的
    assert(USER_ACCESS(start, end));
    ...
    // 按页面为单位复制内存内容
    do {
        // 获取源进程（from）在地址 start 上的页表项
        pte_t *ptep = get_pte(from, start, 0), *nptep;

        // 如果源进程的页表项不存在，则将起始地址向上对齐到下一个页的起始位置，并继续
        if (ptep == NULL) {
            start = ROUNDDOWN(start + PTSIZE, PTSIZE);
            continue;
        }

        // 获取目标进程（to）在地址 start 上的页表项，如果没有页表项，则分配一个新的页表
        if (*ptep & PTE_V) {
            // 如果目标页表没有页表项，则为目标进程分配一个页表项
            if ((nptep = get_pte(to, start, 1)) == NULL) {
                return -E_NO_MEM;  // 内存不足，返回错误
            }

            // 获取源页表项的权限标志（例如用户访问权限）
            ...
            assert(page != NULL);  // 确保源页面不为空
            assert(npage != NULL); // 确保新分配的页面不为空
    ...
    } while (start != 0 && start < end);  // 循环直到复制完成整个范围
    return 0;
}
